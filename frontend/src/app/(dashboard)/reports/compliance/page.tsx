...
import { useSubmissionCompliance } from '@/hooks/use-reports'
...
export default function SubmissionCompliancePage() {
  const [year, setYear] = useState<number>(currentYear)
  const [weeks, setWeeks] = useState<number>(12)

  const { data, isLoading } = useSubmissionCompliance({ year, weeks })

  const sorted = data
    ? [...data].sort((a, b) => Number(a.compliance_rate) - Number(b.compliance_rate))
    : []

  const avgRate = sorted.length > 0
    ? sorted.reduce((s, d) => s + Number(d.compliance_rate), 0) / sorted.length
    : 0
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-2xl font-bold tracking-tight">Submission Compliance</h1>
          <p className="text-sm text-muted-foreground mt-0.5">
            Weekly report submission rates by location
          </p>
        </div>
      </div>
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-muted-foreground">
                  Last {weeks} weeks across {sorted.length} location{sorted.length !== 1 ? 's' : ''}
                </p>
              </div>
            </div>
          </CardContent>
        ...
        <Card>
          <CardHeader className="pb-3">
            <CardTitle className="text-base">
              Compliance by Location (sorted worst first)
            </CardTitle>
          </CardHeader>
          <CardContent>
            <div className="rounded-md border">
              <Table>
                ...
                <TableBody>
                  {sorted.map((row) => {
                    const rate = Number(row.compliance_rate)
                    return (
                      <TableRow key={row.location_id}>
                        ...
                        <TableCell className="text-right font-mono">
                          {Number(row.expected_submissions)}
                        </TableCell>
                        <TableCell className="text-right font-mono">
                          {Number(row.actual_submissions)}
                        </TableCell>
                        <TableCell>
        ...
        <Card>
          <CardContent className="py-12 text-center">
            ...
            <p className="text-sm text-muted-foreground mt-1">
              Compliance data will appear after weekly reports are submitted.
            </p>
          </CardContent>
        </Card>
      )}
